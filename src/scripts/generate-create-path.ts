...
const sourceFile = await readSourceFile('../pages.gen.ts');
...
function isPropertySignature(
	node: ts.Node,
	name?: string,
): node is ts.PropertySignature & Required<Pick<ts.PropertySignature, 'type'>> {
	return (
...
function isStringLiteral(
	node: ts.Node,
): node is Omit<ts.LiteralTypeNode, 'literal'> & { literal: ts.StringLiteral } {
...
if (output) {
	writeOutputFile(output, '../utils/create-path.gen.ts', '../../biome.json');
}
